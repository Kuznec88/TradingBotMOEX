    ...
    while True:
        ...
        try:
            with Client(token=token, target=host) as client:
                ...
                need_reconnect_due_to_stale_md = False
                for event in stream:
                    if _duration_reached():
                        logger.info("[TBANK][MD] run duration reached (%ss), stopping feed loop.", stop_after)
                        return
                    ...
                    # If stream is alive but does not provide actionable market-data updates
                    # (e.g. only ping/unknown/service messages), force reconnect after timeout.
                    md_age_live = time.monotonic() - last_md_update_mono[0]
                    if stale_reconnect_sec > 0 and md_age_live > float(stale_reconnect_sec):
                        need_reconnect_due_to_stale_md = True
                        logger.warning(
                            "[TBANK][MD] no market-data update for %.0fs (>%ss) while stream is active; reconnecting",
                            md_age_live,
                ...
                elif need_reconnect_due_to_stale_md:
                    logger.warning("[TBANK][MD] reconnecting after stale market-data watchdog trigger")
