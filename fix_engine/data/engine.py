...
from fix_engine.config.constants import MD_MID_SPIKE_THRESHOLD_RATIO, MD_SNAPSHOT_LOG_INTERVAL_SEC
...
class MarketDataEngine:
    """
    Хаб рыночных данных: нормализация сырого ввода в MarketData, последний снимок по символу,
    рассылка подписчикам (стратегии, gateway).
    """
    ...
    def __init__(
        ...
        *,
        ...
        mid_spike_threshold_ratio: float | None = None,
    ) -> None:
        self._logger = logger
        self._lock = RLock()
        self._latest: dict[str, MarketData] = {}
        self._subscribers: list[Callable[[MarketData], None]] = []
        self._snapshot_interval_sec = float(snapshot_interval_sec or MD_SNAPSHOT_LOG_INTERVAL_SEC)
        self._spike_threshold = float(mid_spike_threshold_ratio or MD_MID_SPIKE_THRESHOLD_RATIO)
    ...
    def subscribe(self, callback: Callable[[MarketData], None]) -> None:
        """Подписка на каждое обновление MarketData (вызывается из потока стрима)."""
        with self._lock:
            self._subscribers.append(callback)

    def update_market_data(self, raw_data: dict[str, object]) -> MarketData:
        """Принять сырой словарь от провайдера, нормализовать и разослать подписчикам."""
    ...
    def get_health_metrics(self) -> dict[str, float | int | bool]:
        """Счётчики для мониторинга живости MD (потокобезопасный снимок)."""
        with self._lock:
